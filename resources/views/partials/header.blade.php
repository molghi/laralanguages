@php
    ...
    $page_is_add_edit_word = str_contains($current_page, '/form');
...
<header class="{{ config('tailwind.block-bg') }} p-4 text-white font-mono">
    <div class="max-w-4xl mx-auto flex items-center justify-between">
        <div class="text-xl font-bold">Easy Lang Coach</div>
        
        <div class="relative z-[5] flex items-center gap-6">
            @if ($user_id)
                <a href="/" class="bg-green-700 {{ config('tailwind.btn-styles') }} {{ $page_is_add_edit_word ? $current_page_classes : '' }}">Add Word</a>
                <a href="/words" class="bg-blue-700 {{ config('tailwind.btn-styles') }} {{ $page_is_words ? $current_page_classes : '' }}">View Words</a>
                <a href="/" class="bg-orange-500 {{ config('tailwind.btn-styles') }} {{ $page_is_practice ? $current_page_classes : '' }}">Practice</a>
